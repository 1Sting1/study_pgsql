...
-- Создадим тип для результата среднего метео (если требуется использовать тип, например, для дальнейших вызовов)
drop type if exists avg_meteo_result cascade;
create type avg_meteo_result as (
    avg_temperature_deviation numeric(8,2),
    avg_wind_speed numeric(8,2),
    avg_wind_direction numeric(8,2)
);

-- Универсальная хранимая процедура для расчёта средних показателей метеоданных
drop procedure if exists sp_calc_avg_meteo(integer);
create procedure sp_calc_avg_meteo(par_measurement_type_id integer)
language plpgsql
as $$
declare
    ...
    avg_wind_direction numeric(8,2);
begin
    -- Рассчитываем среднее отклонение температуры, среднюю скорость и направление ветра
    select avg(public.fn_calc_header_temperature(temperature))::numeric(8,2),
           avg(wind_speed)::numeric(8,2),
           avg(wind_direction)::numeric(8,2)
    into avg_temp_dev, avg_wind_speed, avg_wind_direction
    from public.measurment_input_params
    where measurment_type_id = par_measurement_type_id;

    raise notice 'Для типа измерения %: Среднее отклонение температуры = %, Средняя скорость ветра = %, Среднее направление ветра = %',
        par_measurement_type_id, avg_temp_dev, avg_wind_speed, avg_wind_direction;
end;
$$;

-- Тестовый расчет: вызов процедуры для ДМК (тип 1) и Ветрового ружья (тип 2)
call sp_calc_avg_meteo(1);
call sp_calc_avg_meteo(2);
